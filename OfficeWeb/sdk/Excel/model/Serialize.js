...
(function ($, window, undefined) {
    ...
    function BinaryWorksheetsTableWriter(memory, wb, oSharedStrings, aDxfs, aXfs, aFonts, aFills, aBorders, aNums, idWorksheet, isCopyPaste) {
        ...
        this._prepeareStyles = function () {
            ...
            this.oFillMap[this._getStringFromObjWithProperty(new Fill())] = {
            ...
            var xfs = {
                borderid: 0,
                fontid: 0,
                fillid: oFillDefElement.index,
                numid: 0,
                align: oAlign,
                QuotePrefix: null
            };
            this.oXfsMap["0|0|" + this.nDefaultFillIndex + "|0|" + sAlign] = {
        ...
        this.WriteSheetView = function (oSheetView) {
            ...
            if (null !== oSheetView.pane && oSheetView.pane.isInit()) {
                this.bs.WriteItem(c_oSer_SheetView.Pane, function () {
                    oThis.WriteSheetViewPane(oSheetView.pane);
                });
            }
        };
        this.WriteSheetViewPane = function (oPane) {
            var oThis = this;
            this.bs.WriteItem(c_oSer_Pane.State, function () {
                oThis.memory.WriteString3(c_oAscPaneState.Frozen);
            });
            this.bs.WriteItem(c_oSer_Pane.TopLeftCell, function () {
                oThis.memory.WriteString3(oPane.topLeftFrozenCell.getID());
            });
            var col = oPane.topLeftFrozenCell.getCol0();
            var row = oPane.topLeftFrozenCell.getRow0();
            if (0 < col) {
                this.bs.WriteItem(c_oSer_Pane.XSplit, function () {
                    oThis.memory.WriteDouble2(col);
                });
            }
            if (0 < row) {
                this.bs.WriteItem(c_oSer_Pane.YSplit, function () {
                    oThis.memory.WriteDouble2(row);
                });
            }
        };
        ...
        this.WriteDrawings = function (aDrawings) {
            ...
            for (var i = 0, length = aDrawings.length; i < length; ++i) {
                ...
                if (!this.isCopyPaste) {
                    ...
                    if (this.isCopyPaste && oDrawing.graphicObject.selected) {
                        if (oDrawing.isGroup() && oDrawing.graphicObject.selectedObjects && oDrawing.graphicObject.selectedObjects.length) {
                            ...
                            for (var selDr = 0; selDr < oDrawingSelected.length; selDr++) {
                                ...
                                this.bs.WriteItem(c_oSerWorksheetsTypes.Drawing, function () {
                                    oThis.WriteDrawing(oDrawing, curDrawing);
                                });
        ...
        this.WriteDrawing = function (oDrawing, curDrawing) {
            ...
            case c_oAscCellAnchorType.cellanchorOneCell:
                ...
                this.bs.WriteItem(c_oSer_DrawingType.Ext, function () {
                    oThis.WriteExt(oDrawing.ext);
                });
                break;
            case c_oAscCellAnchorType.cellanchorAbsolute:
                this.bs.WriteItem(c_oSer_DrawingType.Pos, function () {
                    oThis.WritePos(oDrawing.Pos);
                });
                this.bs.WriteItem(c_oSer_DrawingType.Ext, function () {
                    oThis.WriteExt(oDrawing.ext);
                });
            ...
            if (oDrawing.isChart()) {
                this.bs.WriteItem(c_oSer_DrawingType.GraphicFrame, function () {
                    oThis.WriteGraphicFrame(oDrawing);
                });
            } else {
                if (curDrawing) {
                    this.bs.WriteItem(c_oSer_DrawingType.pptxDrawing, function () {
                        window.global_pptx_content_writer.WriteDrawing(oThis.memory, curDrawing, null, null, null);
                    });
                } else {
                    this.bs.WriteItem(c_oSer_DrawingType.pptxDrawing, function () {
        ...
        this.WriteGraphicFrame = function (oDrawing) {
            ...
            this.bs.WriteItem(c_oSer_DrawingType.Chart2, function () {
                oBinaryChartWriter.WriteCT_ChartSpace(oDrawing.graphicObject);
            });
        };
        ...
        this.WriteCell = function (cell, nXfsId, nRowIndex) {
            var oThis = this;
            if (cell.nCol >= 0) {
                this.bs.WriteItem(c_oSerCellTypes.RefRowCol, function () {
                    oThis.memory.WriteLong(nRowIndex);
                    oThis.memory.WriteLong(cell.nCol);
                });
                if (null != nXfsId) {
        ...
        this.WriteComments = function (aComments, aCommentsCoords) {
            var oThis = this;
            var oNewComments = {},
            i, length, elem, nRow, nCol, row, comment;
    ...
    function Binary_WorksheetTableReader(stream, wb, aSharedStrings, aCellXfs, Dxfs, oMediaArray, copyPasteObj) {
                                                ...
                                                if (c_oSerWorksheetsTypes.Drawings == type) {
                                                    ...
                                                    if (c_oSerWorksheetsTypes.Autofilter == type) {
                                                        ...
                                                        if (c_oSerWorksheetsTypes.TableParts == type) {
                                                            ...
                                                            if (c_oSerWorksheetsTypes.Comments == type && !(typeof editor !== "undefined" && editor.WordControl && editor.WordControl.m_oLogicDocument && Array.isArray(editor.WordControl.m_oLogicDocument.Slides))) {
                                                                res = this.bcr.Read1(length, function (t, l) {
            ...
            if (c_oSerWorksheetPropTypes.Name == type) {
                ...
                if (c_oSerWorksheetPropTypes.SheetId == type) {
                    ...
                    if (c_oSerWorksheetPropTypes.State == type) {
                        ...
                        if (this.copyPasteObj.isCopyPaste && c_oSerWorksheetPropTypes.Ref == type) {
            ...
            if (c_oSerSheetFormatPrTypes.DefaultColWidth == type) {
                ...
                if (c_oSerSheetFormatPrTypes.BaseColWidth === type) {
                    ...
                    if (c_oSerSheetFormatPrTypes.DefaultRowHeight == type) {
                        ...
                        if (c_oSerSheetFormatPrTypes.CustomHeight == type) {
                            var oAllRow = oWorksheet.getAllRow();
                            ...
                            if (c_oSerSheetFormatPrTypes.ZeroHeight == type) {
                                var oAllRow = oWorksheet.getAllRow();
        ...
        this.ReadCell = function (type, length, ws, oCell, nRowIndex) {
            ...
            if (c_oSerCellTypes.Ref == type) {
                ...
                if (c_oSerCellTypes.RefRowCol == type) {
                    var nRow = this.stream.GetULongLE();
                    oCell.nRow = nRowIndex;
                        ...
                        if (c_oSer_DrawingType.Pos == type) {
                            ...
                            if (c_oSer_DrawingType.Ext == type) {
                                ...
                                if (c_oSer_DrawingType.Pic == type) {
                                    ...
                                    if (c_oSer_DrawingType.GraphicFrame == type) {
                                        res = this.bcr.Read1(length, function (t, l) {
